from .db_config import Connection, Users, db
...
def get_friend_requests(user_id):
    ...
    received_friend_requests = db.session.query(Users).filter(Connection.user_b_id == user_id,
                                                             Connection.status == "Requested").join(Connection,
                                                                                                    Connection.user_a_id == Users.user_id).all()

    sent_friend_requests = db.session.query(Users).filter(Connection.user_a_id == user_id,
                                                         Connection.status == "Requested").join(Connection,
                                                                                                Connection.user_b_id == Users.user_id).all()
